...
const currentIdentity = computed(() => `mer:${authorId.value}`)
...
const { data } = await useAsyncData<WorksPageResponse>(
  ...
  async () => {
    ...
    const identity = `mer:${requestedAuthorId}`
    try {
...
useSeoMeta({
  title: () => works.value
    ? `${works.value.author.full_name}, Mer | Litteraturbanken`
  ...
  <div>
    <div v-if="!accepted" class="searching" aria-live="polite">
      <div class="preloader">
        ...
        <span class="sr-only">Laddar texter om författaren</span>
      </div>
    </div>
    ...
    <AuthorWorksContent v-else :response="works" variant="mer" />
  </div>
</template>
