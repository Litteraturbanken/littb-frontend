...
import type { EmbedSession, EmbedStatus } from "~/composables/useReaderDictionaryEmbed"
...
const frameTitle = computed(() =>
    props.mode === "embed" ? `Slå upp ${props.word} i SO och SAOB` : ""
)
</script>
            ...
            <DialogPanel class="modal-content" tabindex="-1">
                <div v-if="props.mode === 'legacy'" class="so_modal">
                    <div class="modal-header">
                        <DialogTitle as="h4"
                            >Svensk ordbok utgiven av <br>Svenska Akademien (2009)</DialogTitle
                        >
                        <button
                            ref="closeButton"
                ...
                <div v-else class="reader-dictionary-embed">
                    <div class="modal-header">
                        <DialogTitle as="h4">Svenska Akademiens ordbok</DialogTitle>
                        <button
                            ref="closeButton"
                            class="btn pull-right"
                            type="button"
                    ...
                    <p
                        v-if="
                            props.fullSiteUrl &&
                            ['empty', 'error', 'timeout'].includes(props.status)
                        "
                        class="reader-dictionary-embed__fallback"
                    >
                        <a :href="props.fullSiteUrl"
                            >Öppna uppslaget på Svenska Akademiens ordbokssida</a
                        >
                    </p>
                </div>
            </DialogPanel>
