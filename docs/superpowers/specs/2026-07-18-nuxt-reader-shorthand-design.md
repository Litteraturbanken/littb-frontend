...
route. Its Reader metadata lookup selects the requested representation and,
when the URL has no `/sida/` segment, replaces the URL with the representation's
`startpagename`. Query state such as bare `?om-boken` survives that replacement.
...
`/api/get_work_info` request as the canonical Reader endpoint. Metadata fetch
and validation move into a small server utility shared by the resolver and
canonical Reader handler so the two paths cannot drift.

The resolver accepts only `etext` in this slice because the canonical Nuxt
Reader currently supports only `etext`. It selects an exact representation
whose `titlepath` and `mediatype` match the route, requires a non-empty
`startpagename`, verifies that start page exists in the validated page list,
...
`useRequestFetch`; no composable is introduced. It strictly validates the
returned identity and canonical path before calling `navigateTo` with
`replace: true` and an SSR redirect status.

The page appends the original raw query suffix from `route.fullPath`, rather
than reconstructing `route.query`, so bare keys, ordering, repeated keys, and
encoding remain unchanged. The route renders no durable UI: successful SSR
requests redirect before HTML, and client navigation immediately replaces the
temporary route. A bounded legacy-style preloader is permitted only for a
client transition that has not yet resolved.
...
1. the resolver sends exactly one queryless `get_work_info` request with the
   exact encoded author/title identity and never fetches Reader HTML;
2. exact `etext` metadata returns the validated start page and canonical path;
3. unsupported media, missing representations/pages, identity mismatches, and
   malformed payloads fail with the specified status;
4. a direct shorthand request returns an SSR redirect to the canonical Reader
   path while preserving the raw query suffix exactly;
5. client navigation from a migrated Library/Author link replaces the
   shorthand URL, renders the canonical Reader, and produces no router,
   hydration, or page errors;
6. canonical Reader, Reader search-hit, Library, Author Works, typecheck,
   production build, and diff/API checks remain green.
