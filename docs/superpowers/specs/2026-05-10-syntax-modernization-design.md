...
- `npm test -- --reporter=list` runs 28 tests and reports 27 passing tests when Playwright starts the Vite dev server.
...
- `npm test -- --reporter=list` reports the expected baseline of 27 passing tests and the SO modal failure, unless that failure is explicitly fixed or quarantined.
...
- Build passes.
- Playwright reports 27 passing tests and one failing SO modal test.
