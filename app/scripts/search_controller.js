...
function SearchPageCtrl(
    ...
    SearchStateService
        ...
        const bridgedProps = [
            "filters", "selectedTitles", "selectedKeywords", "nav_filter",
            "current_page", "advanced", "isAuthorAboutSearch", "searching",
            "sentsWithHeaders", "sentsNavFilter", "num_hits", "authorsById",
            "authors", "titles", "titles_hits", "aboutAuthors", "doc_hits",
            "total_pages", "authorStatsData", "query", "pageTitle",
            "showGotoHitInput", "from_index", "auth_select_rendered",
            "loadingTitles", "loadingAuthors"
        ]
        ...
        this.$onInit = function () {
            ...
            $element.find("select.title_select").on(
                "select2:opening",
                getListener("select.title_select", "loadingTitles", false)
            )
            $element.find("select.author_select").on(
                "select2:opening",
            ...
            ctrl.titleChange = () => {
                let workid = null
                if (ctrl.selected_title && ctrl.selected_title.lbworkid) workid = ctrl.selected_title.lbworkid
            ...
            function refreshTitles(countOnly, filterstr) {
                ...
                let resultlimit = ctrl.filters["authors>authorid"].length ? 10000 : 30
            ...
            const onKeyDown = function (event) {
                if (event.metaKey || event.ctrlKey || event.altKey || document.activeElement?.tagName === "INPUT") {
                    return
                }
