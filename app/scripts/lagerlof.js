 ...
 */
littb.controller("textjamforelseCtrl", function($scope, $animate, $rootScope, $location, $modal, backend, $window, $timeout) {
    const s = $scope;
    s.loading = false;
    s.error = false;
    s.work = null;
    ...
    const makeHTMLold = function(data, myWits) {
        const cleanAppsNew = function(data, myWits) {
            ...
            var makeCache = function(k0, v0, i0, stop) {
              ...
              while (i <= stop) {
                ...
                const kstr = (Array.from(k).map((w) => `#${w}`)).join(' ');
            ...
            const mergeApps = function(app1, app2) {
                ...
                if (app1len === 1 && 1 === app2len) { // both not diff. can merge
                    c0++;
                    app1[0].text += app2[0].text;
                } else if ((app2len > 1) && (app2len === app1len) && (
                  (app2len === 2) || ( function() {
                ...
                } else if ((app2len > 1) && (app1len > 1)) { // both are diff. can merge
                    c1++;
                    const merged = [];
                    for (rdg1 of Array.from(app1)) {
            ...
            while (lastIndex < dataLen) {
                ...
                if (app === null) {
                    ...
                    if (!skipToAppEnd && (result = rdgPat.exec(line))) { // rdg
                        wits = result[1];
                        if ((wits = filterCache[wits]) !== '') {
                            ...
                            if (wits === myWitsStr) {
                                c6++;
                                skipToAppEnd = true;
                                if (text === undefined) { // check for empty reading
        ...
        const makeHtmlFromApps = function(apps) {
            ...
            const pages = ((() => {
                const result = [];
                for (wit of Array.from(myWits)) {                     result.push({wit, n: null, usedInContext: true});
                }
                return result;
            })());
            ...
            for (let appIndex = 0; appIndex < apps.length; appIndex++) {
                ...
                if (app.anchor === 'p') {
                    ...
                    if (!prevIsDiff && !nextIsDiff) {
                        ...
                        if (((bulkStartIndex = nthIndexOf(text, ' ', nWords+1)) !== -1) &&
    ...
    s.submit = function() {
        ...
        c.log("utgåvor:");
        c.log((Array.from(s.worksToCompare).map((w) => (w.title + ", id:" + w.id))).join("\n"));
        ...
        backend.getDiff( workgroup, myWits, ...Array.from(ids) ).then(function(data) {
            // c.time 'makeHTML all'
        ...
        if (!Array.from(myWits).includes(s.baseWit)) { // reset baseWit?
    ...
    s.onClickOutsideContextVersionsDiv = function(evt) {
        ...
        } else if (contextVersionsContext && (evt.target !== contextVersionsContext[0])) { // other click outside
    ...
    s.showContextVersionsDiv = function(contextSpan) {
        const contextVersionsHtml = function() {
            ...
            const result = ((() => {
                const result1 = [];
                for (wit of Array.from(myWits)) {                     result1.push({wit, title: s.witTitles[wit], html: '', url: s.witUrls[wit]});
                }
                return result1;
            })());
...
`;
    ...
    $("#koll-text")
        .on("click", ".koll-context", function(evt) {
            ...
            if ((contextVersionsContext === null) || (contextVersionsContext[0] !== target) || (s.contextVersions === null)) {
                s.showContextVersionsDiv($(target));
                return evt.stopPropagation();
            } // keep ContextVersionsDiv from immediately hiding again
        })
        .on("mouseover", ".koll-changed", function() { return showDiffDiv($(this)); })
