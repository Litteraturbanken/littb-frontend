...
littb.factory("debounce", $timeout =>
    function(func, wait, options) {
        ...
        const delayed = function() {
            ...
            if (trailing) { return result = func.apply(thisArg, args); }
        };
        if (options === true) {
            leading = true;
            trailing = false;
...
littb.factory("util", function($location) {
    ...
    const camelCase = name =>
        name.replace(SPECIAL_CHARS_REGEXP, function(_, separator, letter, offset) {
            if (offset) { return letter.toUpperCase(); } else { return letter; }
    ...
    return {
        getInnerXML(elem) {
            ...
            const strArray = Array.from(elem.childNodes).map((child) =>
                xml2Str(child));
        ...
        titleSort(a) {
           return _.map(a.sortkey.split(/(\d+)/), function(item) { 
               if (Number(item)) {
                   const zeroes = (_.map(__range__(0, (10 - item.toString().length), true), () => "0")).join("");
                ...
                for (let obj of Array.from(config)) {
                    ...
                    result.push(scope.$watch(watch, ((obj, watch) =>
                        function(val) {
                            ...
                            if (obj.replace !== false) { loc.replace(); }
        ...
        setupHash(scope, ...nameConfig) {
            ...
            _.extend(scope, _.pick($location.search(), ...Array.from(names)));
            scope.loc = $location;
            scope.$watch('loc.search()', () => _.extend(scope, _.pick($location.search(), ...Array.from(names))));

            return (() => {
                const result = [];
                for (let name of Array.from(nameConfig)) {
                    var callback;
                    if (_.isObject(name)) {
                        [name, callback] = Array.from(_.head(_.toPairs(name)));
                    }
                    scope[name] = $location.search()[name];
                    result.push(scope.$watch(name, (name =>
                        function(val) {
                            $location.search(name, val || null);
                            if (callback) { return callback(val); }
                        }
                    )(name)
                    ));
                }
...
const expandMediatypes = function(works, mainMediatype) {
    ...
    const makeObj = function(metadata) {
        if (metadata.mediatype === "pdf") {
            ...
            return {
                label : metadata.mediatype,
                url : `/forfattare/${getMainAuthor(metadata).author_id}/titlar/${metadata.work_title_id || metadata.title_id}/sida/${metadata.startpagename}/${metadata.mediatype}`
            };
        }
    };
    ...
    for (let key in groups) {
        ...
        const [main, ...rest] = Array.from(group);
        ...
        // if main.has_epub
        for (let work of Array.from(group)) {
            if (work.has_epub) {
                mediatypes.push({
                    label : "epub",
                    url : `txt/epub/${getMainAuthor(work).author_id}_${work.work_title_id || work.title_id}.epub`,
                    downloadable : true
                });
                break;
            }
...
littb.factory('backend', function($http, $q, util, $timeout, $sce) {
    ...
    const objFromAttrs = function(elem) {
        if (!elem) { return null; }
        return _.fromPairs((Array.from(elem.attributes).map((attrib) => [util.normalize(attrib.name), attrib.value])));
    };
    ...
    return {
        ...
        getParts(filterString, partial_string, text_filter = null) {
            if (partial_string == null) { partial_string = false; }
            const def = $q.defer();
        ...
        getTitles(author,
                     ...
                     include = null,
                     text_filter = null) {
            if (aboutAuthors == null) { aboutAuthors = false; }
        ...
        getProvenance(workinfo) {
            return $http({
                ...
                for (let i = 0; i < iterable.length; i++) {
                    ...
                    let signum = "";
                    if (prov.signum) { signum = ` (${prov.signum})`; }
        ...
        getSourceInfo(params, mediatype) {
            ...
            $http({
                ...
                if (mediatype) {
                    for (let work of Array.from(works)) {
                        if (work.mediatype === mediatype) {
                            workinfo = work;
                            break;
                        }
                    }
                    if (!workinfo) { workinfo = works[0]; }
                } else {
                    workinfo = works[0];
                }
                ...
                for (let pg of Array.from(workinfo.pages)) {
                ...
                workinfo.errata = Array.from($("tr", workinfo.errata)).map((tr) =>
                    _($(tr).find("td")).map(util.getInnerXML)
                    .map(_.str.strip).value());



                workinfo.partStartArray = _(workinfo.parts).map(part => [workinfo.pagemap[`page_${part.startpagename}`], part])
                .sortBy(function(...args) { 
                    const [i, part] = Array.from(args[0]);
                    return i;}).value();
        ...
        getTextByAuthor(author_id, textType, maybeAuthType, list_about) {
            if (list_about == null) { list_about = false; }
            const params = { 
                exclude : "text,parts,sourcedesc,pages,errata",
                to : 10000,
            ...
            return $http({
                ...
                params
            }).then( response => expandMediatypes(response.data.data)
            , err => c.log("err", err));
        },
        ...
        searchLexicon(str, id, useWildcard, doSearchId, strict) {
            ...
            http({
                ...
                let output = Array.from($("artikel", xml)).map((article) => ({
                ...
                output = _.sortBy(output, function(item) {
                    if (item.baseform === str) { return "aaaaaaaaa"; }
        ...
        getBiblinfo(params, wf) {
            ...
            $http({
                ...
                const output = Array.from($("entry", xml)).map((entry) => ({
            ...
            if (!url) {
               def.reject(`${title} not of ${(() => {
                ...
                http({
                    ...
                    const data = [];
                    for (let entry of Array.from($("glossentry", xml))) {
                        const pages = [];
                        try {
                            for (let page of Array.from($("page", entry))) {
        ...
        fetchOverlayData(lbworkid, ix) {
            ...
            return this.getHtmlFile(url).then(function(response) {
                ...
                // c.log $(html)
                const max = _.max(_.map($(html).data("size").split("x"), Number));
        ...
        autocomplete(filterstr) {
            return $http({
                ...
                c.log("suggest!", __guard__(content.suggest != null ? content.suggest[0] : undefined, x => x.text), "score", __guard__(content.suggest != null ? content.suggest[0] : undefined, x1 => x1.score));
                if (!(content.data.length || (content.suggest != null ? content.suggest.length : undefined))) {
                    data = [{
                ...
                for (let item of Array.from(content.data)) {
                    ...
                    if (["etext-part", "faksimil-part"].includes(item.doc_type)) {
                        ...
                        item.label = `${((item.authors != null ? item.authors[0] : undefined) || item.work_authors[0]).surname} – ${item.shorttitle || item.title}`;
                        item.typeLabel = "Del";
...
littb.factory("authors", function(backend, $q) {
    ...
    const def = $q.defer();
    // @promise = def.promise
    backend.getAuthorList(null, (exclude='intro,db_*,doc_type,corpus,es_id,doc_id,doc_type,corpus_id,imported,updated,sources')).then(function(authors) {
    ...
    return (SearchData = class SearchData {
        ...
        submit(query, params) {
            ...
            return $http({
                ...
                const sentsWithHeaders = _.flatten(this.decorateData(response.data.data, this.NUM_HIGHLIGHTS));
        ...
        getMoreHighlights(sentenceData) {
            if (sentenceData.at_highlight_page == null) { sentenceData.at_highlight_page = 1; }
        ...
        decorateData(data, num_fragments) {
            const groupSents = data => {
                ...
                for (let item of Array.from(data)) {
                    ...
                    for (let highlight_index = 0; highlight_index < item.highlight.length; highlight_index++) {
        ...
        compactLeftContext(data) {
            ...
            //         ctx.num_chars = sum
            c.log("min", min);
            return Array.from(data).map((work) =>
                (() => {
                    const result = [];
                    for (let ctx of Array.from(_.map(work.highlight, "left_context"))) {
        ...
        next() {
            if ((this.current + 1) === this.total_doc_hits) { return {then : angular.noop}; }
            this.current++;
        ...
        prev() {
            if (this.current === 0) { return {then : angular.noop}; }
            this.current--;
            return this.get(this.current);
        }
...
littb.factory("SearchWorkData", function(SearchData, $q, $http) {  
    ...
    return (SearchWorkData = class SearchWorkData extends SearchData {
        ...
        submit(query, params) {
            ...
            const source = new EventSource(`${STRIX_URL}/search_document/${params.lbworkid}/${params.mediatype}/${query}/?` + queryParams.join("&"));
            ...
            source.onmessage = event => {
                ...
                return this.scope.$apply(() => {
                    ...
                    if (this.n_times > 1) {
                        ...
                        return this.total_hits = data.total_hits;
                    }
                });
            };
            ...
            source.onerror = function(event) {
                ...
                this.close();
                return self.scope.$apply(() => self.isCounting = false);
            };
                // def.resolve()
