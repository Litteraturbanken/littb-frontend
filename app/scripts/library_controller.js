    ...
    controller($scope, $element, $attrs, $location) {
        ...
        if ($attrs.disableOnKeyword !== undefined) {
            var unwatch = $scope.$watch(
                ...
                val => {
                    if (!val) return
                    $element[0].querySelectorAll("option").forEach(opt => opt.removeAttribute("disabled"))
                    let opts = val.split(",")
                    for (let v of opts) {
...
function LibraryPageCtrl(
    ...
    LibraryStateService
        ...
        ctrl.resetView = function () {
            ...
            $timeout(() => angular.element(document.querySelectorAll(".gender_select, .keyword_select, .about_select")).select2(), 0)
            ctrl.filter = ""
            ctrl.rowfilter = ""
        ...
        ctrl.fetchWorks = (countOnly, epubOnly, isSearchRecent, pdfOnly) => {
            ...
            let filterString = ctrl.rowfilter
            if (ctrl.dl_mode) {
