...
littb.controller("MenuCtrl", function($scope, util) {
    ...
    const s = $scope;
    if (s.$root.collapsed == null) { s.$root.collapsed = [true, true, true, true, true, true, true, true]; }
...
littb.controller("lyrikTeacherCtrl", ($scope, util, $location) => c.log("location", $location.url()));


const getStudentCtrl = id =>
    ["$scope", "$routeParams", "$rootElement", "$location", function($scope, $routeParams, $rootElement, $location) {
...
littb.controller("fileCtrl", function($scope, $routeParams, $location, $anchorScroll, $q, $timeout, $rootScope, $rootElement) {
...
littb.directive("scFile", ($routeParams, $location, $http, $compile, util, backend) =>
    ({
        ...
        link($scope, elem, attr) {
            ...
            const generalTextsForLyrikStudents = [
            ...
            if (subsection === "gymnasium") { subsection = "gy"; }

            if (Array.from(generalTexts).includes(filename)) {
                path = `/skola/${filename}`;
            } else if (Array.from(generalTextsForLyrikStudents).includes(filename) && _.startsWith($location.url(), "/skola/lyrik/elev")) {
            ...
            if ((section === "larare") && subsection) {
                ...
                const s = $(`a[href='${actualPath}']`).parent().parent().scope();
                if (s) {
                    safeApply(s, function(s) {
                        const index = $(`a[href='${actualPath}']`).parent().parent().attr("collapse").slice(-2, -1);
...
littb.directive("activeStyle", ($routeParams, $timeout) =>
    ({
        link($scope, elem, attr) {
            const selected = elem.find("a[href$='html']").removeClass("selected")
            .filter(`[href$='${$scope.docurl}']`).addClass("selected");
            c.log("selected", selected);

        
            return $timeout(() => $scope.setName(selected.last().text())
            , 0);
        }
    })
);
    ...
    ({
        link($scope, elem, attr) {
            ...
            if (_.str.endsWith(href, $scope.docurl)) {
                ...
                return $timeout(() => $scope.setName(($interpolate(elem.text()))($scope))
                , 0);
            }
        }
    })
