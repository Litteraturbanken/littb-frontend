...
>>>>>>> fiddled around with the school
  ...
  littb.run(function($rootScope, $location, $rootElement, $q, $timeout) {
    var firstRoute, normalizeUrl, stripClass;
    firstRoute = $q.defer();
  ...
  littb.filter("setMarkee", function() {
    return function(input, fromid, toid) {
      ...
      if (fromid === toid) {
        ...
        $("#" + fromid, input).nextUntil("#" + toid, "span").andSelf().add("#" + toid, input).addClass("markee").filter(":odd").addClass("flip");
      }
