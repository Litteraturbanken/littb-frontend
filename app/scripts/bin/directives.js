(function() {
  ...
  littb.directive('breadcrumb', function($interpolate, $rootScope) {
    return {
      restrict: "EA",
      link: function(scope, elem, attrs) {
        var a, i, watches, _fn, _i, _len, _ref;
        elem.remove();
        watches = [];
        _ref = elem.children();
        _fn = function(a, i) {
          var inplHref, inplText;
          inplText = $interpolate($(a).text());
          if ($(a).attr("href")) {
            inplHref = $interpolate($(a).attr("href"));
          }
          watches.push(scope.$watch(function(s) {
            return inplText(s);
          }, function(label) {
            if (!label) {
              return;
          ...
          if (inplHref) {
            return watches.push(scope.$watch(function(s) {
              return inplHref(s);
            }, function(url) {
              if (!$rootScope.breadcrumb[i]) {
                $rootScope.breadcrumb[i] = {};
        ...
        return scope.$on("$destroy", function() {
          var wtch, _j, _len1, _results;
          _results = [];
          for (_j = 0, _len1 = watches.length; _j < _len1; _j++) {
            wtch = watches[_j];
            _results.push(wtch());
          }
          return _results;
