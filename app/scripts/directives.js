    ...
    ({
        ...
        link(scope, element, attrs, controller) {
            ...
            return scope.$on("$destroy", function() {
                if (attrs.toolkitReplace) {
                    return element.replaceWith(replaced);
                } else {
    ...
    ({
        ...
        compile(elm, attrs) {
            ...
            return function(scope, iElement, iAttrs) {
                ...
                return scope.$on("$destroy", () => $("#reading_css").attr("href", null));
            };
        }
    })
);
    ...
    ({
        ...
        compile(elm, attrs) {
            ...
            return (scope, iElement, iAttrs) =>
                scope.$watch("title", val => $("title").text(val))
            ;
        }
    })
);
...
<div class="sort" ng-class="{disabled : !active}">
        ...
        link(scope, elem, iAttrs) {
            ...
            const tupMatches = tup =>
                _.every(_.map(_.zip(val, tup), function(...args) {
                    const [item1, item2] = Array.from(args[0]);
                    return item1 === item2;
                })
                )
            ;
            s.$watch("tuple", function(newtup) {
                const [newval, dir] = Array.from(newtup);
                s.active = tupMatches(newval);
                c.log("active", s.active);
                return s.enabled = [!dir, dir];
        });
            s.up = () => s.tuple = [val, true];
            return s.down = () => s.tuple = [val, false];
        }
    })
);
...
littb.directive('square', () =>
    ({
        ...
        link(scope, elm, attrs) {
            ...
            coors = _.fromPairs(_.map(coors, function(val, key) {
                val = Number(val);
...
littb.directive('clickOutside', $document =>
    ({
        ...
        link(scope, elem, attr, ctrl) {
            ...
            let skip = false;
            elem.bind('click', (handler1 = function(e) {
                skip = true;
            })
            );
            
            $document.bind('click', (handler = function(e) {
                if (!skip) {
                    scope.$eval(attr.clickOutside, {$event:e}); // event object can be accessed as $event, as with ng-click 
                }
                skip = false;
            })
            ); //# HO! watch out! not to implicitly return false
...
littb.directive('scrollTo', ($window, $timeout) => 
    ...
    ({
        link(scope, elem, attr) {
            return scope.$watch(( () => scope.$eval(elem.attr("scroll-to"))), function(val) {
                if (!val) { return; }
    ...
    ({
        ...
        link(scope, elem, attr) {
            return scope.$watch(( () => elem.find(".in.collapsing").height()), function(val) {
                ...
                return scope.index = (__guard__(elem.find(".in.collapsing").scope(), x => x.$eval("$index")));
            });
        }
    })
);
    ...
    ({
        ...
        link(scope, elem, attrs) {
            ...
            return scope.$watch("lex()", function(val) {
                if (!val) { return; }
                return __guard__(elem.find(`#${val}`).get(0), x => x.scrollIntoView());
            });
        }
    })
);
        ...
        link(scope, elem, attr) {
            return elem.on("click", function() {
                ...
                if (id) {
                    ...
                    c.log("click id", id);
                    return scope.$emit("search_dict", null, id, true);
                } else {
                    // $location.search("lex", null)
                    c.log("click not id", elem);
                    return scope.$emit("search_dict", _.str.trim(elem.text(), null, false));
                }
            });
        }
    })
    ...
    ({
        link(scope, elem, attr) {
            ...
            const showIndicator = function(target) {
                ...
                return box = $(`<div><i class='fa fa-search glass'></i> \
...
<span class='circle'></span></div>`)
            ...
            return elem.on("mouseup", _.debounce( function(event) {
                let needle;
                const sel = typeof $window.getSelection === 'function' ? $window.getSelection().toString() : undefined;
                const isOneWord = sel && (needle = " ", !Array.from(_.str.trim(sel)).includes(needle));
...
`,
        ...
        link(scope, elem, attr) {
            scope.text = null;
            scope.show = false;
            return $rootScope.$on("notify", function(event, text) {
                scope.text = text;
                scope.show = true;
                return $timeout(() => scope.show = false
                , 4000);
            });
        }
    })
...
littb.directive("typeaheadTrigger", () =>
    ({
        ...
        link(scope, element, attr, ctrls) {
            return scope.$on('open', (event, value) => ctrls[0].$setViewValue(value));
        }
    })
);
...
littb.directive('metaDesc', $interpolate =>
    ({
        ...
        link(scope, elm, attrs) {
            ...
            const wtch = scope.$watch(s => inpl(s)
            , val => $("meta[name=description]").attr("content", val));

            return scope.$on("$destroy", () => wtch());
        }
    })
);
...
littb.directive('pageTitle', $interpolate =>
    ({
        ...
        link(scope, elm, attrs) {
            ...
            const wtch = scope.$watch(s => inpl(s)
            , function(val) {
                if (val) { val += " | Litteraturbanken"; }
                return $("head > title").text(val || "Litteraturbanken");
            });
...
littb.directive('breadcrumb', ($interpolate, $rootScope) =>
    ({
        ...
        link(scope, elem, attrs) {
            ...
            elem.remove();
            const watches = [];
            const iterable = elem.children();
            for (let i = 0; i < iterable.length; i++) {
                const a = iterable[i];
                (function(a, i) {
                    let inplHref;
                    const inplText = $interpolate($(a).text());
                    if ($(a).attr("href")) {
                    ...
                    , function(label) {
                        if (!label) { return; }
                        if (!$rootScope.breadcrumb[i]) {
                            $rootScope.breadcrumb[i] = {};
                        }
                        return $rootScope.breadcrumb[i].label = label;
                    })
                    );
                    if (inplHref) {
                        return watches.push(scope.$watch(s => inplHref(s)
                        , function(url) {
                            if (!$rootScope.breadcrumb[i]) {
                                $rootScope.breadcrumb[i] = {};
                            }
                            return $rootScope.breadcrumb[i].url = url;
                        })
                        );
                    }
                })(a, i);
...
`,
        link(scope, elem, attr) {
            c.log("attr", attr);
            return scope.getUrl = function(filename) {
...
littb.directive("schoolAffix", $window =>
    ...
    ({
        ...
        link(scope, elem, attrs) {
            ...
            const killDetect = detectScrollDir(() => detach(elem.height())
            , reset);
        ...
        link(scope, elem, attrs) {
            ...
            return (() => {
                ...
                for (let key in obj) {
                    const val = obj[key];
                    if (val) {
                        result.push(elem.addClass(key));
                    } else {
                        result.push(elem.removeClass(key));
                    }
                }
                return result;
            })();
    ...
    ({
        ...
        link(scope, elem, attr) {
            ...
            return elem.on("click", () =>
                safeApply(scope, () => scope.top = __guard__(elem.position(), x => x.top))
            );
        }
    })
);            
...
`,
        ...
        link(scope, element, attr) {
            ...
            return scope.$on("$routeChangeStart", function(event, next, current) {
                ...
                return blockRemoveBkg = current.$$route.school && next.$$route.school;
            });
        }
    })
);
...
const overflowLoad = function(s, element) {
    ...
    return element.load(function() {
        ...
        if ($(this).width() < actualWidth) {
            ...
            return btn = $("<button class='btn btn-xs expand'>Förstora</button>").click(() => s.$emit("img_expand", element.attr("src"), actualWidth)).appendTo(element.parent());
        } else {
            if (btn != null) {
                btn.remove();
            }
            return element.parent().removeClass("img-overflow");
        }
    });
};
        ...
        compile(elm, attrs) {
            if (_.str.endsWith(elm.attr("src"), ".svg")) {
                elm.load(elm.attr("src"), function(data) {
                    let __, height, width;
                    [__, __, width, height] = Array.from(data.match(/viewBox="(.+?)"/)[1].split(" "));
                    elm.width(width);
...
`,
        link($scope, element, attr) {
            ...
            return s.searchOptSelect = function(sel) {
                const o = s.searchOptionsMenu;

                const currents = _.filter((_.values(o)), "selected");
                const isDeselect = Array.from(currents).includes(sel);
                const deselectAll = () =>
                    Array.from(currents).map((item) =>
                        (item.selected = false))
                ;
                ...
                if (["prefix", "suffix", "infix"].includes(sel.val) && (currents.length === 1) && isDeselect) {
                ...
                if (["prefix", "suffix"].includes(sel.val)) {
                    ...
                    if (isDeselect) { o.infix.selected = false; }
                }
