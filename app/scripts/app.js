...
if (location.hash.length && _.startsWith(location.hash, "#!%2F")) { //rewrite for incoming #! with encoded url
    location.href = decodeURIComponent(location.href).replace("/#!/", "/");
} else if (location.hash.length && _.startsWith(location.hash, "#!/")) { //rewrite for incoming #!
...
const authorResolve = ["$q", "$routeParams", "$route",
                            function($q, $routeParams, $route) {
                                const def = $q.defer();
                                c.log("resolve", $routeParams, $route);
                                if (((routeStartCurrent != null ? routeStartCurrent.$$route.controller : undefined) === "authorInfoCtrl") && 
    ...
    .config(function($routeProvider) {
        ...
        window.Router = class Router {
            ...
            when(route, obj) {
                ...
                for (let r of Array.from(route)) {
                    if (r.split("/")[1] === "forfattare") {
                        const shortRoute = r.replace(/^\/forfattare\//, "/f/").replace("/titlar/", "/t/");
            ...
            otherwise() { return $routeProvider.otherwise.apply($routeProvider, arguments); }
        };
                ...
                controller : ["$scope", "$routeParams", "$http", "util", "$rootElement",
                                function($scope, $routeParams, $http, util, $rootElement) {
                                    ...
                                    return $http.get(`/red/presentationer/${$routeParams.folder}/${$routeParams.doc}`).success(function(data) {
                                        ...
                                        return $scope.setTitle($scope.title);
                                    });
                                }
                ],
                resolve : { 
                    r : ["$q", "$routeParams", "$route", "$rootScope",
                            function($q, $routeParams, $route, $rootScope) {
                                ...
                                if (((routeStartCurrent != null ? routeStartCurrent.$$route : undefined) === $route.current.$$route) &&
...
`
                ...
                // title : "Aktuellt"
                {redirectTo() { return "/bibliotek?nytillkommet"; }})
            .when('/nytt',
                {redirectTo() { return "/bibliotek?nytillkommet"; }})
            .when('/om/:page', {
                ...
                resolve : { 
                    r : ["$q", "$routeParams", "$route", "$rootScope",
                            function($q, $routeParams, $route, $rootScope) {
                                ...
                                if (((routeStartCurrent != null ? routeStartCurrent.$$route.controller : undefined) === "aboutCtrl") && 
            ...
            .when(['/dramawebben', '/dramawebben/pjäser', '/dramawebben/författare', '/dramawebben/om','/dramawebben/kringtexter'], {
                ...
                resolve: {
                    r: ["$q", "$routeParams", "$route",
                            function($q, $routeParams, $route) {
                                const def = $q.defer();
                                if (((routeStartCurrent != null ? routeStartCurrent.$$route.controller : undefined) === "dramawebCtrl") && 
            ...
            .when(["/forfattare/LagerlofS",
                ...
                template : "<div></div>",
                controller : ["$scope", "backend", "$routeParams", "$location", function($scope, backend, $routeParams, $location) {
                    const params = {
                        authorid: $routeParams.author,
                        titlepath : $routeParams.title
                    };
                    return backend.getSourceInfo(params).then(data => $location.url(`/forfattare/${$routeParams.author}/titlar/${$routeParams.title}/sida/${data.startpagename}/${data.mediatype}?om-boken`).replace());
                }
                ]
            })
                ...
                resolve : {
                    r : ["$q", "$routeParams", "$route", "$rootScope",
                            function($q, $routeParams, $route, $rootScope) {
                                ...
                                if (((routeStartCurrent != null ? routeStartCurrent.$$route.controller : undefined) === "readingCtrl") && ($route.current.controller === "readingCtrl")) {
                                    const cmp = ["author", "mediatype", "title"];
                                    if ("lbid" in $route.current.params) {
                                        cmp.push("lbid");
                                    }
                                    const current = _.pick($route.current.params, ...Array.from(cmp));
                                    const prev = _.pick(routeStartCurrent.params, ...Array.from(cmp));
...
littb.run(function($rootScope, $location, $rootElement, $q, $timeout, bkgConf) {
    ...
    $timeout( () => $rootElement.addClass("ready").removeClass("not_ready")
    , 1000);
    ...
    const stripClass = function(prefix) {
        ...
        cls = cls.replace(re, "");
        return $rootElement.attr("class", cls);
    };
    ...
    $rootScope.$on("$routeChangeSuccess", function(event, newRoute, prevRoute) {
        ...
        $("#confObjStyle").text("");
        stripClass("bkg");
        return bkgConf.get(path).then(function(confObj) {
            ...
            if (confObj) {
                ...
                if (confObj["class"]) {
                    return (() => {
                        const result = [];
                        for (className of Array.from(confObj["class"].split(" "))) {
                            result.push($("body").addClass(`bkg-${className}`));
                        }
                        return result;
                    })();
                }
...
*/
    ...
    return normalizeUrl = function(str) {
        const trans = _.fromPairs(_.zip("åäö", "aao"));

        return _.map(str, letter => trans[letter.toLowerCase()] || letter);
    };
});
...
littb.filter("numberFmt", () =>
    function(input) {
        ...
        if (input.toString().length < 5) { return input; }
...
littb.filter("normalizeAuthor", function() {
    let trans = _.fromPairs(_.zip("ÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝàáâãäåçèéêëìíîïñòóôõöøùúûüýÿ", "AAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy"));
