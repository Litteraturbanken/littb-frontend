...
- **Total Tests**: 32 tests across 10 describe blocks
...
Current `Syntax` baseline: 31 of 32 tests pass. The known failing test is `Reader > should show SO modal`, which depends on the SO modal flow and should be fixed or quarantined separately.
...
**Required**: Current `Syntax` baseline is 32 total tests, with 31 passing and known failing test `Reader > should show SO modal` unless it has been fixed or quarantined.
...
**Command**: `npx playwright test`
**Required**: Current `Syntax` baseline applies: 31 of 32 tests pass, with known failing test `Reader > should show SO modal` unless fixed or quarantined.
...
**Command**: `npx playwright test`
**Required**: Current `Syntax` baseline applies: 31 of 32 tests pass, with known failing test `Reader > should show SO modal` unless fixed or quarantined; include performance validation.
...
4. Only proceed when the suite is back to the current baseline: 32 total tests, 31 passing, with only `Reader > should show SO modal` failing unless it has been fixed or quarantined.
